var _ = {
	...
	_arrayEach: function (array, fn, ctx) {
		for (var i = 0, length = array.length; i < length; i += 1) {
			fn.call(ctx, array[i], i);
		}
	},

	_objEach: function (obj, fn, ctx) {
		var keys = Object.keys(obj);
		for (var i = 0, length = keys.length; i < length; i += 1) {
			fn.call(ctx, obj[keys[i]], i);
		}
	}
	...
	each: function (obj, fn, ctx) {
		if (Array.isArray(obj)) {
			this._arrayEach(obj, fn, ctx);
		} else if (this.isObject(obj)) {
			this._objEach(obj, fn, ctx);
		}
	},
...
ISSchemaTools = function (obj, pattern) {
