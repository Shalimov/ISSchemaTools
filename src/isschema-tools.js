(function () {
    ...
    var _ = {
        ...
        isEqualArrays: function (first, second) {
            if (first === second) {
                return true;
            }
            ...
            do {
                if (first[flength] === second[flength]) {
                    continue;
                }
                return false;
            } while ((flength--) >= 0);
        ...
        initial: function (array) {
            return Array.isArray(array) ? array.slice(0, array.length - 1) : array;
        },
        ...
        extend: function (obj, extendObj) {
            if (!(this.isObject(obj) || this.isObject(extendObj))) {
                return;
            }
    ...
    var ISSchemaTools = (function () {
        ...
        Rule.createName = function (path) {
            return path.filter(isNotNumber).join('.');
        };

        function Node(value, key, path, level, parent, pattern) {
            this.key = key;
            this.value = value;
            this.level = level;
            this.path = path || [];
        ...
        function traverse(obj, fn) {
            ...
            var circularDepend = [];
            var stack = [new Node(obj, null, null, 1, null, null)];
            ...
            function iterate(val, key) {
                ...
                if (_.isObject(val)) {
                    ...
                    var node = new Node(val, key, path, level, this, null);
        ...
        function prepareNode(val, key, pattern) {
            ...
            pattern.name = pattern.name || Rule.createName(path);

            return new Node(val, key, path, level, null, pattern);
        }

        function matchTraverse(obj, pattern) {
            if (!(_.isObject(obj) && _.isObject(pattern))) {
                throw new Error('First and Second arguments should be an Object');
            }

            var nodes = [];
            var stack = [
                {
                    node: new Node(obj, null, null, 1, null, pattern),
                    stage: pattern
                }
            ];
            ...
            function iterate(stage, key) {
                var value;
                var pattern = this.pattern[_.isNumber(key) ? 0 : key];
                ...
                if (pattern instanceof Rule) {
                    ...
                    nodes.push(prepareNode.call(this, value, key, pattern));
    ...
    ISSchemaTools.defineExtension('transform', function (_) {
        ...
        function iterateNode(node) {
            ...
            if (Array.isArray(transformers)) {
                for (var i = 0, length = transformers.length; i < length; i++) {
                    node.value = transformers[i](node.value, node.key, node.pattern.type);
                }
            }
        }

        function transform() {
            _.each(this._nodes, iterateNode);
            return this;
        }
    ...
    ISSchemaTools.defineExtension('validation', function (_) {
        ...
        function NodeValueInjector(nodes, node) {
            this._nodes = nodes;
        ...
        _.extend(NodeValueInjector.prototype, {
            ...
            _getSiblingValue: function (expression) {
                ...
                for (var i = nodes.length - 1; i >= 0; i--) {
                    if (_.isEqualArrays(nodes[i].path, siblingPath)) {
                        return nodes[i];
                    }
                }
            },
            _getValuesByName: function (expression) {
                ...
                for (var i = nodes.length - 1; i >= 0; i--) {
                    if (nodes[i].pattern.name === expression) {
            ...
            get: function (expression) {
                ...
                return this._getValuesByName(expression);
            }
        });
        ...
        function validate(options) {
            ...
            _.each(nodes, function iterateNode(node) {
                ...
                for (var i = 0, length = keys.length; i < length; i += 1) {
                    ...
                    if (validator(value, args, new NodeValueInjector(nodes, node), pattern) === true) {
                        continue;
                    }
        ...
        register('equalTo', function (val, ruleName, injector) {
            return val === _.first(injector.get(ruleName));
        });
